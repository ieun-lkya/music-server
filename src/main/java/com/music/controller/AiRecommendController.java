...
public class AiRecommendController {
    ...
    public Result<List<Map<String, Object>>> generatePlaylists() {
        ...
        try {
            ...
            //  终极 Prompt：彻底解除数量封印，要求全局遍历匹配！
            String prompt = String.format(
                    "你是一个极具灵魂深度和音乐审美的AI主理人。当前随机时空指纹为：【%s】。\n" +
                    "请极其仔细地阅读下方的【曲库全集】：\n%s\n" +
                    "【你的使命】：\n" +
                    "1. 请完全摆脱任何已知的音乐分类模板（如赛博、治愈等），利用你庞大的知识图谱，**自发创造** 4 个独特、震撼的歌单主题。\n" +
                    "2. 为每个歌单撰写一个【分类依据(basis)】：深层剖析这几首歌在编曲、意境或抽象情感上的内在联系（不少于40字）。\n" +
                    "3.  核心任务：遍历上方曲库中的**所有**歌曲，把**所有**符合该主题意境的歌曲 ID 全部挑选出来放入 ids 数组！绝对不要遗漏，符合多少首就放多少首，上不封顶！\n" +
